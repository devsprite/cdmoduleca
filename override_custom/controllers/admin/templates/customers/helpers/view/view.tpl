...
{if Tools::getValue('send_message') == 1}
	<div class="alert alert-success">
			<button class="close" data-dismiss="alert" type="button">×</button>
				{l s='Message send with success'}
	</div>
{/if}
				...
				{if count($discounts)}
					<table class="table">
						<thead>
							<tr>
								...
								<th><span class="title_box ">{l s='Name'}</span></th>
								<th><span class="title_box ">{l s='Montant Réduction'}</span></th>
								<th><span class="title_box ">{l s='Réduction %'}</span></th>
								<th><span class="title_box ">{l s='Status'}</span></th>
							<tr/>
						</thead>
						<tbody>
					{foreach $discounts AS $key => $discount}
							<tr>
								...
								<td>{$discount['code']}</td>
								<td>{$discount['name']}</td>
								<td>{$discount['reduction_amount']}</td>
								<td>{$discount['reduction_percent']}</td>
								<td>
									{if $discount['active']}
										<i class="icon-ok"></i>
									{else}
...
<!-- added for sending messages-->
			...
			<div class="panel">
				...
				<div id="messages" class="well hidden-print">
					<form action="{$smarty.server.REQUEST_URI|escape:'html':'UTF-8'}&amp;token={$smarty.get.token|escape:'html':'UTF-8'}" method="post" onsubmit="if (getE('visibility').checked == true) return confirm('{l s='Do you want to send this message to the customer?'}');" enctype="multipart/form-data" >
						<div id="message" class="form-horizontal">

					{*		<div class="form-group">
					*			<div class="col-lg-6">
					*			   <select name="id_order" class="form-control">
					*					<option value="0">{l s='-- Choose the order to link the messages --'}</option>
...
<!-- end added for sending messages-->
			...
			<div class="panel">
				...
				<div id="" class="well hidden-print">
						...
						<a class="btn btn-default" target="_blank" href="?controller=AdminOrders&addorder&createorderfromcustomer&id_customer={$customer->id|intval}&token={getAdminToken tab='AdminOrders'}">
							<i class="icon-edit"></i>
